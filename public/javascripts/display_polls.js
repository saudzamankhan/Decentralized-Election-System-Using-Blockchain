...
{
  $.get("http://localhost:3000/fetchpollingstations").done(function(jdata) {
...
function RefreshTable()
{
    ...
    datatable.draw();
       ...
       $(document).ready(function() {
        ...
        $('#editpollid').val(rowData.pollid);
        $('#editpollcity').val(rowData.city);
        ...
        $.ajax({
          type: 'POST',    
          url:'http://localhost:3000/deletepollingstation',
          data:'deletepollid='+ rowData.pollid,
      ...
      $(document).on('click', '#addformpress', function() {
        $.ajax({
          type: 'POST',    
          url:'http://localhost:3000/appendpollingstation',
          data:'addpollid='+ $('#addpollid').val() + '&addpollcity=' + $('#addpollcity').val(),
          success: function(msg) {
      ...
      $(document).on('click', '#updateformpress', function() {
        $.ajax({
          type: 'POST',    
          url:'http://localhost:3000/updatepollingstation',
          data:'editpollid='+ $('#editpollid').val() + '&editpollcity=' + $('#editpollcity').val(),
          success: function(msg) {
